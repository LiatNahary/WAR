...
//
//
public class TestIt extends JFrame implements ActionListener {
    ...
    private void guiSetCounters() {
        cardCounterPlayer.setText("Card Count: " + (gm.getPlayerDeckSize()));
        cardCounterOpponent.setText("Card Count: " + (gm.getOpponentDeckSize()));
    }

    private void guiDealCards(JTextField textFieldPlayer, JTextField textFieldOpponent ) {
        textFieldPlayer.setText(gm.cardName(gm.getPlayerTopCard()));
        textFieldOpponent.setText(gm.cardName(gm.getOpponentTopCard()));
    }
    ...
    public void playHand() {
        ...
        setPlayerTextBoxesBlank();
        setOpponentTextBoxesBlank();
        if (gm.testForWinner() == GameManager.RoundWinner.DRAW) {
            if (!gm.areConditionsForWarMet()){
                endGame();
            }
            guiDealCards(dealtCardPlayer,dealtCardOpponent);
            gm.addToWarStack();
            guiDealCards(warCardIPlayer, warCardIOpponent);
            gm.addToWarStack();
            guiDealCards(warCardIIPlayer, warCardIIOpponent);
            gm.addToWarStack();
           guiDealCards(warCardIIIPlayer, warCardIIIOpponent);
            if (gm.testForWinner() == GameManager.RoundWinner.DRAW) {
                    playHand();
            }
            if (gm.testForWinner() == GameManager.RoundWinner.PLAYER){
                    gm.playerWinRound();
                    guiSetCounters();
                    return;
            }
    ...
    public void endGame() {
        GameManager.RoundWinner stopGame = gm.stopGame();
        switch (stopGame){
            case PLAYER:
                setPlayerTextBoxesBlank();
    ...
    public void guiSetup() {
        ...
        labelPlayer.setText(" Worthy Player");
        ...
        warCardIIIPlayer.setFont(new Font(FONT_TYPE, Font.PLAIN, 20));
        warCardIIIPlayer.setForeground(new Color(48, 77, 94));
        ...
        warCardIIIOpponent.setFont(new Font(FONT_TYPE, Font.PLAIN, 20));
        warCardIIIOpponent.setForeground(new Color(145, 53, 51));
